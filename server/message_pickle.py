...
# Send a message from authorised_user to the channel specified by channel_id automatically at a
...
def message_send(token, channel_id, message):
    ...
    for cha in channelDict:
        if cha['channel_id'] == channel_id:
            if u_id not in cha['channel_owner'] and u_id not in cha['channel_member']:
...
# Given a message within a channel the authorised user is part of, add a "react" to that
# particular message
# ValueError when:
...
# Given a message within a channel, mark it as "pinned" to be given special display treatment by
# the frontend
# ValueError when:
